...
/**
 * Productos con stock bajo para alertas de admin (solo de ESTA tienda).
 * Filtra por products.store_id además del umbral para evitar cruces entre
 * tenants (service-role no aplica RLS).
...
export async function getLowStockItems(storeId) {
  ...
  try {
    const { data, error } = await client
      .from('product_skus')
      .select('product_id, stock, products!inner(store_id)')
      .eq('active', true)
      .eq('products.store_id', storeId)
      .lte('stock', STOCK_LOW_THRESHOLD);
    if (error) return [];
    return (data || []).map((r) => ({ product_id: r.product_id, stock: r.stock }));
  } catch {
    return [];
  }
}
